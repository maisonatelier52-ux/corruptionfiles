...
import { Bell, Calendar, Facebook, Twitter, Instagram, Youtube, Tag } from "lucide-react";
...
/**
 * Convert a relative image path from JSON to an absolute URL.
 * OG / Twitter card crawlers require fully absolute URLs.
 */
...
function CategoryPill({ category }) {
  ...
  return (
    <span
      className={`${meta.color} text-white text-[10px] font-bold px-2 py-0.5 uppercase tracking-wide`}
    >
      {meta.label}
    </span>
  );
...
// ─── NewsListCard ─────────────────────────────────────────────────────────────
// FIX: <h2> → <h3> (page H1 is author name; article cards must be h3)
  ...
  return (
    <article className="flex flex-col sm:flex-row border-b border-gray-200 pb-4 mb-6 last:border-0 last:mb-0">
      ...
      <div className="flex-1 sm:pl-4 pt-3 sm:pt-0">
        ...
        {/* FIX: was <h2> — changed to <h3> (page H1 is the author name) */}
        <h3 className="font-bold text-gray-900 text-base md:text-[17px] leading-snug mb-1">
          <Link
            href={href}
            title={a.title}
        ...
        {CATEGORY_META[a.category] && (
          <Link
            href={`/${a.category}`}
            title={`Browse ${CATEGORY_META[a.category].label} articles`}
        ...
        <p className="text-xs text-gray-500 mb-2">
          ...
          <Link
            ...
            title={`More articles by ${a.authorName}`}
            rel="author"
...
// ─── LatestCard ───────────────────────────────────────────────────────────────
// FIX: title attr, aria-hidden on icons, <time dateTime>
...
// ─── SidebarCategoryCard ──────────────────────────────────────────────────────
// FIX: title attr, descriptive alt on Image
...
// ─── AuthorHeader ─────────────────────────────────────────────────────────────
// FIX: H1 is author name (correct); added aria-label + title on social links;
//      author avatar alt text made descriptive; Tag icon aria-hidden;
//      category link has title attr
      ...
      <div className="flex flex-col md:flex-row md:items-start gap-8">
        ...
        <div className="flex flex-col items-start gap-4 flex-shrink-0">
          <div className="flex flex-row items-center gap-5">
            <div
              className={`relative w-24 h-24 rounded-full overflow-hidden flex-shrink-0 ring-4 ring-offset-2 ${ringColor} shadow-md`}
            >
              <Image
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {catMeta && (
              <Link
                ...
                // FIX: title attr on category link
                title={`Browse ${catMeta.label} articles`}
            ...
            {Object.keys(author.social || {}).map((platformKey) => {
              ...
              return (
                <a
                  ...
                  // FIX: descriptive aria-label and title on social links
                  aria-label={`${author.name} on ${platformLabel}`}
                  title={`Follow ${author.name} on ${platformLabel}`}
                  className={`group transition-colors duration-200 ${
                    hoverColors[platformKey.toLowerCase()] || "hover:text-blue-600"
                  }`}
                >
...
// ─── METADATA ────────────────────────────────────────────────────────────────
// FIX: added keywords, canonical, robots, complete OG (type "profile", absolute
//      avatar image URL + dimensions), Twitter card
...
export async function generateMetadata({ params }) {
  ...
  const bio      = data?.bio ?? `Read the latest investigative articles by ${name} on ${SITE_NAME}.`;
  const url      = `${SITE_URL}/authors/${slug}`;

  // Absolute URL for author avatar — used in OG + Twitter
  const avatarUrl = absoluteImageUrl(data?.avatar || "/og-default.jpg");

  return {
    title: `${name} — ${catLabel || "Author"} | ${SITE_NAME}`,
    description: bio,

    // FIX: keywords per author
    keywords: `${name}, ${catLabel || "investigative journalist"}, ${SITE_NAME}, author, journalism`,
    ...
    // "profile" type is correct for author/person pages
    openGraph: {
      type: "profile",
      url,
      siteName: SITE_NAME,
      title: `${name} — ${catLabel || "Author"} | ${SITE_NAME}`,
      description: bio,
      // FIX: absolute avatar URL so social crawlers can show the author photo
      images: [
        {
          url: avatarUrl,
          secureUrl: avatarUrl,
          width: 400,
          height: 400,
          alt: `${name} — Author at ${SITE_NAME}`,
        },
      ],
      // OG profile-specific fields
      firstName: name.split(" ")[0],
      lastName:  name.split(" ").slice(1).join(" ") || undefined,
      username:  slug,
    },

    // FIX: Twitter card with absolute author image
    twitter: {
      card: "summary",
      title: `${name} — ${catLabel || "Author"} | ${SITE_NAME}`,
      description: bio,
...
export default async function AuthorPage({ params }) {
  ...
  const authorData = getAuthorBySlug(authorSlug) ?? {
    ...
    role:       "Contributor",
    bio:        `${authorSlug} is a contributor at ${SITE_NAME}.`,
    social:     {},
    category:   null,
  };
  ...
  const pageUrl     = `${SITE_URL}/authors/${authorSlug}`;
  const avatarUrl   = absoluteImageUrl(authorData.avatar || authorData.coverImage);

  // ─── JSON-LD ────────────────────────────────────────────────────────────────
  // FIX: Complete ProfilePage + Person schema
  // FIX: added sameAs (social links), image as ImageObject, jobTitle,
  //      worksFor, knowsAbout, publisher block, BreadcrumbList
  ...
  const profileJsonLd = {
    ...
    mainEntity: {
      ...
      url:         pageUrl,
      jobTitle:    authorData.role || "Journalist",

      // FIX: image as proper ImageObject (not a plain string)
      image: {
        "@type": "ImageObject",
        url:    avatarUrl,
        width:  400,
      ...
      // FIX: worksFor links person to the organization
      worksFor: {
      ...
      // FIX: knowsAbout from their category
      knowsAbout: catMeta?.label
        ? [catMeta.label, "Investigative Journalism"]
        : ["Investigative Journalism"],

      // FIX: sameAs populates author's social profiles for Google Knowledge Graph
      sameAs: Object.values(authorData.social || {}).filter(
        (v) => typeof v === "string" && v.startsWith("http")
      ),
    },

    // FIX: publisher block required by Google
    publisher: {
      "@type": "NewsMediaOrganization",
      name: SITE_NAME,
      url:  SITE_URL,
      ...
      <div className="max-w-7xl mx-auto px-4 pt-8 pb-20">
        ...
        <nav aria-label="Breadcrumb" className="mb-4 text-xs text-gray-500">
          <ol className="flex items-center gap-1 flex-wrap">
            ...
            <li aria-hidden="true"><span className="text-gray-300 mx-1">›</span></li>
            <li>
              <Link
                href="/authors"
                title="All Authors"
            ...
            <li aria-current="page">
              <span className="text-gray-700">{authorData.name}</span>
            </li>
          </ol>
        </nav>

        {/* AuthorHeader contains the page H1 (author name) */}
        <AuthorHeader author={authorData} articleCount={articles.length} />

        {/* ── Articles section heading ─────────────────────────────────── */}
        {/* FIX: changed to <h2> — correct under page H1 (author name) */}
        <div
          className={`border-b-2 mb-8 pb-1 ${catMeta ? catMeta.border : "border-black"}`}
        >
          <h2 className="text-[24px] font-bold text-black leading-tight">
            Articles by{" "}
            <span className={catMeta ? catMeta.text : "text-black"}>
              {authorData.name}
            </span>
          </h2>
